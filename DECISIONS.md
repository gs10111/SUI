...
4. **ESCOPO.** `dir` afeta os dois EIXOS DO PRODUTO, cada um pelo seu proprio parametro, e e aplicado por eixo (`PresetWizard::reading`) e nao por registrador. **EMENDA 2026-09-17:** o canal Y do produto passou a ser alimentado pelo registrador 2 (ANG_Z) em vez do 1 (ANG_Y) - ver docs/protocolo-rs485.md secao 14 - entao `dir` do eixo Y age hoje sobre o ANG_Z. O texto original desta decisao dizia que `dir` nao afeta o registrador 2; com a troca de eixo isso deixou de valer, e a frase fica corrigida aqui em vez de virar contradicao silenciosa. `dir` continua sem afetar o registrador 4 (TEMP) e a leitura crua usada pelas guardas de faixa de D11 item 7 — a banda de tolerancia e a falha mecanica sao avaliadas sobre `|bruto|`, antes de `dir` e antes de `offset`, porque sao propriedades da MONTAGEM, nao da convencao de sinal.
