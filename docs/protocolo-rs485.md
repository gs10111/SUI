...
|---|---|---|---|
| **P1** | **Bloqueador.** A sensora **boota falando o quadro do jig**, nao Modbus (`g_activeProtocol = &g_jigSlave`, `sensor/src/main.cpp`). A troca so acontece pelo comando `proto modbus` no console USB e **nao persiste** (nao ha NVS na sensora) | Confirmado por leitura de codigo | Apos **cada** ciclo de energia, reset do STWD100 ou brownout, a sensora ignora em silencio todo pedido Modbus. A UR fica em falha permanente |
