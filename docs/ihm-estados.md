...
| ▲ com hold | **sem funcao** em todos os estados, exceto ▲ mantida na energizacao (L234). Um gesto sem funcao declarada deve ser **ignorado**, nunca reinterpretado como toque curto | `[PEND D1]` |
| ▼ com hold | **3000 ms em Modo Normal (B1/B2/B3): autoteste sob demanda do display**, sem senha, saindo ao primeiro toque em qualquer tecla ou por timeout de 30000 ms. Sem funcao nos demais estados, e ali continua sendo ignorado | D12 item 8, **aprovada** (DECISIONS.md:2323); revoga a leitura anterior de que ▼ com hold nao tinha funcao em estado nenhum |
...
| **B7 FALHA_COMM** | substitui a area de leitura, em duas linhas e **sem piscar** (DECISIONS.md:1734). Os quatro pares aprovados em A7 / decisao 7 item 6 (DECISIONS.md:1730-1733): AGUARDANDO -> linha 1 `AGUARDANDO SENSOR`, linha 2 vazia; COMUNICACAO -> `FALHA DE COMUNICACAO` / `Verifique cabo RS485 e +5V`; SENSOR -> `FALHA DO SENSOR` / `Sensor de inclinacao em falha`; INSTAVEL -> `MEDICAO INSTAVEL`, linha 2 vazia. Com o latch de flapping armado (A7, DECISIONS.md:277) a linha 2 vira `FALHA TRAVADA - REARMAR NO MENU`; com o enlace ja recuperado e o latch ainda armado essa string sobe para a linha 1 e a tela **continua** sendo B7, porque os quatro reles continuam em alarme ate o rearme | - | abre a tela de login -> C1 (unica rota ate o rearme com o latch armado) | - | PSET **recusado** (guarda de dado valido) | alterna a tela, se houver | recuperacao -> B1, e so com o latch desarmado | Textos **aprovados**; L297 promete "a mensagem de falha de comunicação" e **nao da o texto** — entra na errata do item 7 |
...
1. ~~**Texto da segunda linha da tela de falha de comunicacao (B7).**~~ **RESOLVIDO por A7, que aprovou a lista da decisao 7 item 6 (DECISIONS.md:1730-1733).** Vale `Verifique cabo RS485 e +5V` — sem `do sensor`, que era a grafia do rascunho de D7 e nunca foi a aprovada. A proposta de D8 (`SENSOR SI-DI141389XY`) foi descartada. O manual continua nao publicando o texto (L297): entra na errata do item 7.
